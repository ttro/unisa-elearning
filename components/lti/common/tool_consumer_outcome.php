...
require_once(__DIR__."/../util/lti_util.php");
...
$oauth_consumer_key = $ltiConfig['key'];
$oauth_consumer_secret = $ltiConfig['secret'];

if ( strlen($oauth_consumer_key) < 1 || strlen($oauth_consumer_secret) < 1 ) {
   //johno commented this out
...
if ( $header_key != $oauth_consumer_key ) {
   ...
   foreach ($headers as $header => $value) {
    ...
    //file_put_contents(__DIR__.'/header.txt', $http_headers);
	//johno commented this out
...
try {
	...
	$file = __DIR__.'/assessment_score.xml';
	file_put_contents($file, $body);
    
	require 'http://unisatest.cloudapp.net/FBN1502/score.php';
